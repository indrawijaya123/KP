							...
							<form action="<?= base_url('admin/stok/add')?>" method="post">
								...
								<form action="<?= base_url('admin/stok/edit/'.sanitasi($id))?>" method="post">
									...
									<div class="form-group">
										<label for="namaStok">Nama Stok <span class="text-danger">*</span></label>
										<input type="text" name="namaStok" id="namaStok" value="<?= set_value('namaStok', sanitasi($dataStok))?>" maxlength="11" class="form-control <?= (form_error('namaStok') != null)? 'is-invalid':''; ?>" placeholder="Nama Stok" required/>
										<?= form_error('namaStok'); ?>
									</div>
									<div class="form-group">
